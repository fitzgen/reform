...
var CheckMultipleField = genericFieldConstructor("CheckMultipleField");
CheckMultipleField.prototype = new BaseChoiceField();
CheckMultipleField.prototype.clean = function (data) {
    var thisField = this;
    if (Object.prototype.toString.call(data) === "[object Array]") {
        return data.map(function (item) {
            var result = BaseChoiceField.prototype.clean.call(thisField, item);
            if (result === null)
                throw new FormValidationError("Bad choice value.");
            return result;
        });
    }
    else {
        return BaseChoiceField.prototype.clean.call(thisField, data);
    }
};
CheckMultipleField.prototype.template = [
    '<(( tag ))>',
    '<label>(( label ))</label>',
    '<ul>',
    '(( choices ))',
...
CheckMultipleField.prototype.choiceTemplate = [
    '<li>',
    '<label for="id_(( name ))_(( i ))">',
    '<input type="checkbox" name="(( name ))" id="id_(( name ))_(( i ))" value="(( choiceVal ))" />',
    '(( choiceLabel ))</label>',
    '</li>'
].join("\n");
exports.CheckMultipleField = CheckMultipleField;
...
Form.prototype.toHtml = function (tagStyle) {
    ...
    for (field in this.fields) {
        this.fields[field].prefix = this.prefix;
